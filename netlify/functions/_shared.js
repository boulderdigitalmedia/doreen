...
// web-push validates the VAPID key format synchronously and throws on
// anything malformed (wrong length, not proper URL-safe base64, etc).
// This runs at module load, so an unguarded throw here would crash EVERY
// function that imports _shared.js at cold start — including ones like
// verify-gift-password.js and gift-manifest.js that only need `sb` for a
// plain database read and never touch push notifications at all. A bad
// or rotated VAPID key should only break push sending, not basic reads
// across the whole app (see the matching guard around the Twilio client
// below — same reasoning, same failure mode).
try {
...
// Same isolation as the webpush guard above: the Twilio SDK validates
// accountSid's format (must start with "AC") at construction time and
// throws synchronously if it doesn't — a blank/rotated/malformed
// TWILIO_ACCOUNT_SID would otherwise crash every function that imports
// _shared.js, not just SMS sending. sendSms() below checks for a null
// client and fails with a clear message instead of a cryptic
...
try {
  ...
  console.error('Twilio client init failed — SMS sending is disabled until this is fixed:', e.message);
}
...
async function sendRenewalReminderEmail(profile) {
  try {
    ...
    // Gift Pack copy leads with the recipient's side of it — they've come
    // to expect these notes; renewing/upgrading keeps that going, rather
    // than framing this as a plain billing/expiration notice. Same
    // framing as sendUpgradeNudgeEmail below, just closer to the deadline.
    // Annual's copy is unchanged (no upgrade path to steer them toward —
    // it's already the top plan).
    const subject = isGiftPack
      ? 'Keep showing up for them'
      : `Your term ends ${termEndLabel} — renew to keep it going`;

    const bodyHtml = isGiftPack
      ? `<p style="font-size:16px;">Your 30-day term ends on <strong>${termEndLabel}</strong>. By now, they've probably come to expect these notes — something small that shows up and reminds them you're thinking of them.</p>
        <p style="font-size:16px;">Renew (or upgrade to annual for $45 instead of $59) to keep that going. Everything you've already sent stays visible to them either way — this is just about what comes next.</p>`
      : `<p style="font-size:16px;">Your current 12-month term ends on <strong>${termEndLabel}</strong>.</p>
        <p style="font-size:16px;">Your annual plan is a one-time yearly payment, so it won't renew on its own — you'll need to come back and check out again for a new 12-month term. Once it ends, your gift(s) stop sending new notes — though everything already sent stays visible to your recipient(s).</p>`;

    const ctaLabel = isGiftPack ? 'Keep it going →' : 'Renew from your account →';
    ...
    await resend.emails.send({
      ...
      html: `<div style="font-family:'DM Sans',Arial,sans-serif;color:#2c3a2e;max-width:520px;margin:0 auto;padding:24px;">
        ${bodyHtml}
        <p><a href="${accountUrl}" style="display:inline-block;background:#7a9e7e;color:#fff;text-decoration:none;padding:12px 22px;border-radius:10px;font-size:14px;">${ctaLabel}</a></p>
      </div>`,
    });
  } catch (err) {
...
async function sendUpgradeNudgeEmail(profile) {
  try {
    ...
    await resend.emails.send({
      from:    buildFrom('A Note For You'),
      to:      buyerEmail,
      subject: `Give them a full year of this`,
      html: `<div style="font-family:'DM Sans',Arial,sans-serif;color:#2c3a2e;max-width:520px;margin:0 auto;padding:24px;">
        <p style="font-size:16px;">By now, they've probably started looking forward to it — a note showing up, proof that you're thinking of them, even on an ordinary day.</p>
        <p style="font-size:16px;">Your 30-Day Gift Pack ends on <strong>${termEndLabel}</strong>. Upgrade to the annual plan and that keeps going for a full year instead of stopping in a few weeks — and you lock in <strong>$45</strong> instead of the usual $59, a standing discount just for Gift Pack buyers.</p>
        <p style="font-size:16px;">This offer never expires, so there's no rush — but if you like what this has become for them, now's an easy time to keep it going.</p>
        <p><a href="${accountUrl}" style="display:inline-block;background:#7a9e7e;color:#fff;text-decoration:none;padding:12px 22px;border-radius:10px;font-size:14px;">Give them a full year — $45 →</a></p>
      </div>`,
    });
  } catch (err) {
